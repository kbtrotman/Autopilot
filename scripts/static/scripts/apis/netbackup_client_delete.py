...
try:
    ...
    debug_logging_top_level.send_log(f"Deleting Standard client from policy {policy_name} with Client {client_name} and Schedule {schedule_name} to Netbackup.")
    nbu_api_top_level.delete_client(policy_name, client_name, config.API_KEY, config.BASE_URL, config.CONTENT_TYPE)
